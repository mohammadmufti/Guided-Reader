...
class Token(TypedDict):
    ...
    iraab: Annotated[
        str | None,
        Doc(
            "The token's case or mood IN THIS POSITION — مرفوع, منصوب, مجرور or "
            "مجزوم — or null. On the TOKEN and not the lexicon entry because "
            "case belongs to a position: the same form is marfu` in one hadith "
            "and majrur in the next. SET ONLY WHERE THE ENDING IS VOWELLED. The "
            "analyser READS the printed mark rather than inferring the case "
            "from syntax: masking that mark drops it from 96.1% to 52.1% and "
            "cuts coverage twelvefold. So this NAMES the editor's own vowel, "
            "and the analyser's contribution is knowing whether the mark is a "
            "case ending at all — a past-tense verb is mabni on fatha, a "
            "diptote takes fatha in the genitive. Where the ending is bare, "
            "nothing is claimed. Re-measure with pipeline/eval_grammar.py."
        ),
    ]
